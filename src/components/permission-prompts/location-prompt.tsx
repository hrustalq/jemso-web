...
import { MapPin, Navigation, Target, X, Loader2, Check, AlertCircle } from "lucide-react";
...
export function LocationPrompt({ 
  ...
  const t = useTranslations("Permissions.location");
  const tCommon = useTranslations("Permissions.common");
  
  const { requestLocation, saveLocation, dismissPrompt, isPromptDismissed, location } = useLocationPreferences();
  const [state, setState] = useState<PromptState>("idle");

  // Don't show if already dismissed or location is set
  if (isPromptDismissed || location) {
    return null;
  }
  ...
  const benefits = [
    { icon: Navigation, text: t("benefits.nearby") },
    { icon: Target, text: t("benefits.distance") },
    { icon: MapPin, text: t("benefits.recommendations") },
  ];
