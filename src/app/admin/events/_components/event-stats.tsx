...
export async function EventStats() {
  const stats = await api.event.events.getStats();
    ...
    <>
      ...
      <Card>
        ...
        <CardContent>
          ...
          <p className="text-xs text-muted-foreground">Будущие события</p>
        </CardContent>
      </Card>

      <Card>
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          <CardTitle className="text-sm font-medium">Прошедшие</CardTitle>
      ...
      <Card>
        ...
        <CardContent>
          ...
          <p className="text-xs text-muted-foreground">
            {Math.round((stats.published / stats.total) * 100)}% от общего числа
          </p>
        </CardContent>
      </Card>
