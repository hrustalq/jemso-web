    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="mt-2 text-muted-foreground">
          Новости доступные для вашего тарифа (Tier {userTier})
        </p>
      </div>
          ...
          <div className="flex-1">
            ...
            <p className="text-sm text-muted-foreground">
              {user?.subscription 
                ...
                : "Оформите подписку для доступа к премиум новостям"
              }
            </p>
          </div>
          {!user?.subscription && (
      ...
      {accessibleNews.length > 0 && (
        <div className="space-y-4">
          <h2 className="text-xl font-semibold flex items-center gap-2">
            ...
            Доступные новости ({accessibleNews.length})
          </h2>
          <div className="space-y-4">
            {accessibleNews.map((item) => (
              <NewsCard key={item.id} news={item} isAccessible />
            ))}
          </div>
        </div>
      )}
      ...
      {lockedNews.length > 0 && (
        <div className="space-y-4">
          <h2 className="text-xl font-semibold flex items-center gap-2 text-muted-foreground">
            ...
            Премиум новости ({lockedNews.length})
          </h2>
          <p className="text-sm text-muted-foreground">
            Повысьте уровень подписки для доступа к этим новостям
          </p>
          <div className="space-y-4">
            {lockedNews.slice(0, 3).map((item) => (
              <NewsCard key={item.id} news={item} isAccessible={false} />
            ))}
          </div>
        </div>
      )}
...
function NewsCard({ news, isAccessible }: NewsCardProps) {
  const content = (
